...
static uldlcounter_t last_custom_rlc_bytes[MAX_MOBILES_PER_GNB] = {0};
static uldlcounter_t last_custom_pdcp_bytes[MAX_MOBILES_PER_GNB] = {0};
...
/* Our custom metric for efficiency calculation */
static meas_record_lst_t fill_Custom_Metric(uint32_t gran_period_ms, cudu_ue_info_pair_t ue_info, const size_t ue_idx)
{
    // Initialize the measurement record with zero
    meas_record_lst_t meas_record = {0};

    // Get RLC stats for a specific Data Radio Bearer (DRB)
    const int rb_id = 1;  // Currently supporting only one DRB
    nr_rlc_statistics_t rlc = get_rlc_stats_per_drb(ue_info.ue->rnti, rb_id);
    nr_pdcp_statistics_t pdcp = get_pdcp_stats_per_drb(ue_info.rrc_ue_id, rb_id);

    // Mark the record value as a real number
    meas_record.value = REAL_MEAS_VALUE;

    // Calculate the downlink data volume at the RLC layer
    // Convert transmitted RLC bytes to kilobits (kb) for the given granularity
    double rlc_volume_dl = (double)(rlc.txpdu_bytes - last_custom_rlc_bytes[ue_idx].dl) * 8 / 1000;  // [kb]
    last_custom_rlc_bytes[ue_idx].dl = rlc.txpdu_bytes;

    // Calculate the downlink data volume at the PDCP layer
    // Convert received PDCP bytes to kilobits (kb)
    double pdcp_volume_dl = (double)(pdcp.rxsdu_bytes - last_custom_pdcp_bytes[ue_idx].dl) * 8 / 1000;  // [kb]
    last_custom_pdcp_bytes[ue_idx].dl = pdcp.rxsdu_bytes;

    // Calculate the ratio of RLC data volume to PDCP data volume
    // This represents the efficiency of data delivery from RLC to PDCP
    if (pdcp_volume_dl != 0) {
        meas_record.real_val = rlc_volume_dl / pdcp_volume_dl;
    } else {
        meas_record.real_val = 0;  // Avoid division by zero
    }
...
static kv_measure_t lst_measure[] = {
  ...
  {.key = "RLC2PDCPEff", .value = fill_Custom_Metric }, 
#endif
}; 
